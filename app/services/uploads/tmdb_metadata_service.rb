...
module Uploads
  ...
  class TmdbMetadataService < ApplicationService
    ...
    def apply_movie_metadata
      data = TheMovieDb::Movie.new(id: video_blob.tmdb_id).results
    ...
    def apply_tv_metadata
      data = TheMovieDb::Tv.new(id: video_blob.tmdb_id).results
    ...
    def fetch_episode_title
      return unless video_blob.season_number && video_blob.episode_number

      season_data = TheMovieDb::Season.new(tv_id: video_blob.tmdb_id, season_number: video_blob.season_number).results
      season_data["episodes"]
        &.find { |episode| episode["episode_number"] == video_blob.episode_number }
        &.dig("name")
        &.to_s
        &.presence
    end
