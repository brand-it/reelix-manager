...
# ---------------------------------------------------------------------------
# Production guard - seeds are for development/testing only
# ---------------------------------------------------------------------------
if Rails.env.production?
  raise 'db:seed is disabled in production. Use manual migrations or production-specific seed tasks.'
end

# ---------------------------------------------------------------------------
# Core OAuth application (all environments)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fake library data (development/test only)
# ---------------------------------------------------------------------------
# ── Movies ────────────────────────────────────────────────────────────────
seed_movies = [
...
# ---------------------------------------------------------------------------
# Tus upload session seed data
# ---------------------------------------------------------------------------
puts '', 'Seeding tus upload session examples...'

# Get reelix OAuth application
reelix_app = Doorkeeper::Application.find_by(uid: 'reelix-client')

# Generate UUIDs for tus upload session IDs (string primary key)
active_id = SecureRandom.uuid
complete_id = SecureRandom.uuid
expired_id = SecureRandom.uuid
finalized_id = SecureRandom.uuid

# Active upload (in progress)
TusUploadSession.find_or_create_by!(fingerprint: 'tus_upload_active') do |s|
  s.id = active_id
  s.filename = 'Inception (2010).mkv'
  s.upload_length = 5_368_709_120 # 5GB
  s.mime_type = 'video/x-matroska'
  s.metadata = '{"upload_name":"Inception (2010).mkv"}'
  s.updated_at = 1.hour.ago # Still active (within 48h expiration)
  s.created_at = 2.hours.ago
end

# Complete upload (ready to finalize)
TusUploadSession.find_or_create_by!(fingerprint: 'tus_upload_complete') do |s|
  s.id = complete_id
  s.filename = 'The Matrix (1999).mkv'
  s.upload_length = 2_147_483_648 # 2GB
  s.mime_type = 'video/x-matroska'
  s.metadata = '{"upload_name":"The Matrix (1999).mkv"}'
  s.updated_at = 30.minutes.ago
  s.created_at = 1.hour.ago
end

# Expired upload (for cleanup job testing)
TusUploadSession.find_or_create_by!(fingerprint: 'tus_upload_expired') do |s|
  s.id = expired_id
  s.filename = 'Old Upload (2020).mkv'
  s.upload_length = 1_073_741_824 # 1GB
  s.mime_type = 'video/x-matroska'
  s.metadata = '{"upload_name":"Old Upload (2020).mkv"}'
  s.updated_at = 3.days.ago # Expired (beyond 48h + 1h buffer)
  s.created_at = 5.days.ago
end

# Finalized upload (linked to VideoBlob)
finalized_blob = VideoBlob.find_by(title: 'Pulp Fiction') || VideoBlob.first
if finalized_blob
  TusUploadSession.find_or_create_by!(fingerprint: 'tus_upload_finalized') do |s|
    s.id = finalized_id
    s.filename = 'Pulp Fiction (1994).mkv'
    s.upload_length = 3_221_225_472 # 3GB
    s.mime_type = 'video/x-matroska'
    s.metadata = '{"upload_name":"Pulp Fiction (1994).mkv"}'
    s.video_blob = finalized_blob
    s.finalized = true
...
puts "Seeded #{TusUploadSession.count} tus upload sessions"
puts "  - Active: #{TusUploadSession.where(finalized: false, updated_at: 49.hours.ago..).count}"
puts "  - Expired: #{TusUploadSession.where(finalized: false, updated_at: ..49.hours.ago).count}"
puts "  - Finalized: #{TusUploadSession.where(finalized: true).count}"
