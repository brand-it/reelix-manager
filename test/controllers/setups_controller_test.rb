...
class SetupsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'POST /setup creates admin user and signs them in when no users exist' do
    ...
    assert_difference 'User.count', 1 do
      post setup_path,
           params: { user: { email: 'newadmin@example.com', password: 'password', password_confirmation: 'password' } }
    end
  ...
  test 'POST /setup is blocked when users already exist' do
    ...
    assert_no_difference 'User.count' do
      post setup_path,
           params: { user: { email: 'hacker@example.com', password: 'password', password_confirmation: 'password' } }
    end
    assert_redirected_to root_path
  end
